...
$(gcloud compute instances list --filter="(name:node-*)" | grep -v NAME | awk -v bastion_ip="$BASTION_EXTERNAL_IP" '{printf "%s ansible_host=%s ip=%s ansible_ssh_common_args=\x27-o ProxyCommand=\"ssh -W %%h:%%p -q " bastion_ip "\"\x27\n",$1,$4,$4;}')
$(gcloud compute instances list --filter="(name:nfs-*)" | grep -v NAME | awk -v bastion_ip="$BASTION_EXTERNAL_IP" '{printf "%s ansible_host=%s ip=%s ansible_ssh_common_args=\x27-o ProxyCommand=\"ssh -W %%h:%%p -q " bastion_ip "\"\x27\n",$1,$4,$4;}')
